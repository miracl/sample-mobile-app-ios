...
- (IBAction)onClickSendButton:(id)sender {
    ...
    if([_user getState] == STARTED_REGISTRATION) {
        ...
        dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^ {
            MpinStatus *mpinStatus = [MPinMFA FinishRegistration:_user pin0:strPIN pin1:nil];
    ...
    else if([_user getState] == REGISTERED)
    {
        ...
        dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^ {
            ...
            MpinStatus *mpinStatus = [MPinMFA FinishAuthentication:_user pin:strPIN pin1:nil accessCode:_accessCode authzCode:&strAuthzCode];
            dispatch_async(dispatch_get_main_queue(), ^ (void) {
